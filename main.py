...
def train(model_type, model, train_dataloader, test_dataloader, optimizer, loss_fn, config):
    ...
    for epoch in tqdm(range(config['epochs'])):
        ...
        #     optimizer.zero_grad()
        #     text, label = list(batch[0]), batch[1].to(device)
        #     input_text = model.tokenizer(text, padding=True, truncation=True, max_length=512, return_tensors="pt").to(device)
        #     # outputs = model.model(**input_text)
        #     outputs = model.forward(input_text)
        #     print('output done')
        ...
        for batch, (review, sentiment) in enumerate(train_dataloader):
            
            
            review, sentiment = review.to(device), sentiment.to(device)
            outputs = model.forward(review)
            # input_text = model.tokenizer(text, padding=True, truncation=True, max_length=512, return_tensors="pt").to(device)
            # outputs = model.model(**input_text)
            # outputs = model.forward(input_text)
            # print('output done')
        ...
        pred = []
        labels = []
        for review, sentiment in test_dataloader:
            review, sentiment = review.to(device), sentiment.to(device)
            # input_text = model.tokenizer(text, padding=True, truncation=True, max_length=512, return_tensors="pt").to(device)
            with torch.no_grad():
                outputs = model.forward(review)
            pred.append(torch.argmax(outputs, dim=-1).cpu().numpy())
            labels.append(sentiment.cpu().numpy())
