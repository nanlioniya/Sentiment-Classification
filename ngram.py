...
class Ngram:
    ...
    def get_ngram(self, corpus_tokenize: List[List[str]]):
        ...
        for tokenized_review in tqdm(corpus_tokenize):
            words = [w for w in tokenized_review]
            for i in range(len(words) - 1):
                model[words[i]][words[i+1]] += 1
                features[(words[i], words[i+1])] += 1
    ...
    def train_sentiment(self, df_train, df_test):
        ...
        feature_num = 500
        most_num_features = self.features.most_common(feature_num)
        features = [f[0] for f in most_num_features]
        ...
        # train_corpus_embedding = np.zeros(shape=(len(df_train),feature_num))
        # for review in df_train['review']:
        #     review_tokenized = ['[CLS]'] + self.tokenize(review)
        #     all_ngram = nltk.ngrams(review_tokenized, 2)
        #     for i, f in enumerate(features):
        #         if f in all_ngram: 
        #             train_corpus_embedding[i] += 1
    
        
        train_corpus_embedding = []
        for review in tqdm(df_train['review']):
            train_tokenize = ['[CLS]'] + self.tokenize(review)
            train_embedding = []
            for f in features:
                count = 0
                for ng in nltk.ngrams(train_tokenize, 2):
                    if f == ng:
                        count += 1
                train_embedding.append(count)
            train_corpus_embedding.append(train_embedding)
        
        test_corpus_embedding = []
        for review in tqdm(df_test['review']):
            test_tokenize = ['[CLS]'] + self.tokenize(review)
            test_embedding = []
            for f in features:
                count = 0
                for ng in nltk.ngrams(test_tokenize, 2):
                    if f == ng:
                        count += 1
                test_embedding.append(count)
            test_corpus_embedding.append(test_embedding)
